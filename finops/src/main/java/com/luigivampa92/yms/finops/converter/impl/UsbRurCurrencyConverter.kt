...
internal class UsbRurCurrencyConverter(private val exchangeRate: Double) : CurrencyConverter {

    override fun convert(record: Record): Record {
        check(record.currency == Currency.USD)

        val amount = record.amount.toDouble() * exchangeRate
        val type = if (amount < 0) OperationType.EXPENSE else OperationType.REVENUE
        return Record(type, amount.toInt(), Currency.RUR)
    }
}